...
def create_files_sequential_zip64(zip_name, inside_name, inside_format):
    ...
    prefix_len = len(f"FILE-{0:08d}:")   # убрали .encode() — len строки == len bytes для ASCII
    ...
    # ── Создаём chunk один раз до цикла ──────────────────────────────
    chunk = bytearray(bytes_per_file)
    # Padding заполняем один раз — он никогда не меняется
    chunk[prefix_len:] = b'0' * (bytes_per_file - prefix_len)
    # ─────────────────────────────────────────────────────────────────

    start = time.time()
    report_every = max(1, count // 100)

    with open(f'{zip_name}.zip', 'wb', buffering=WRITE_BUF) as f:
        f.write(b'\x00' * local_header_size)
        compressor = isal_zlib.compressobj(level=2, wbits=-15)
        final_crc        = 0
        ...
        for i in range(count):
            print(i)
            # ── Только перезаписываем префикс — padding уже на месте ─
            chunk[:prefix_len] = f"FILE-{i:08d}:".encode()
            # ─────────────────────────────────────────────────────────

            final_crc = isal_zlib.crc32(chunk, final_crc)
        ...
        f.write(struct.pack('<4sQ2H2L4Q',
            b'PK\x06\x06', 44, 45, 45, 0, 0,
            1, 1, cd_size, cd_offset))

        f.write(struct.pack('<4sLQL',
            b'PK\x06\x07', 0, cd_offset + cd_size, 1))

        f.write(struct.pack('<4s4H2LH',
            b'PK\x05\x06',
            0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF,
            0xFFFFFFFF, 0xFFFFFFFF, 0))

        f.seek(0)
        f.write(struct.pack('<4s2B4HL2L2H',
            b'PK\x03\x04', 45, 0, 0, 8, 0, 0,
            final_crc,
            0xFFFFFFFF, 0xFFFFFFFF,
            len(fname_bytes), extra_local_len))
        f.write(fname_bytes)
...
create_files_sequential_zip64(file_name, zip_inside_name, zip_inside_format)