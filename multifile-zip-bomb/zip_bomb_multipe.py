...
# ── Імпорт Rust-розширения с fallback ────────────────────────────────
...
try:
    ...
    _HAS_RUST = False
    print(f"backend: pure Python (fallback). Reason: {e}")

# ══════════════════════════════════════════════════════════════════════
#  crc32_combine — потрібен тільки для Python-fallback
# ══════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════
#  ZIP-структури (потрібні тільки для Python-fallback і write pass)
# ══════════════════════════════════════════════════════════════════════
...
def make_eocd(num_entries: int, cd_size: int, cd_offset: int) -> bytes:
    if num_entries > 0xFFFF or cd_size > 0xFFFFFFFF or cd_offset > 0xFFFFFFFF:
        ...
        # Звичайний EOCD (зі значеннями 0xFFFF/0xFFFFFFFF як маркер ZIP64)
        eocd = struct.pack(
            '<IHHHHIIH',
            0x06054b50,
            0, 0,
            0xFFFF,              # маркер: дивись ZIP64
            0xFFFF,
            0xFFFFFFFF,          # маркер: дивись ZIP64
            0xFFFFFFFF,
            0,
        )
